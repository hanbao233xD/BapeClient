...
import static mc.bape.util.utils.RenderUtils.drawCircle;
import static mc.bape.util.utils.RenderUtils.drawImage;
import static org.lwjgl.opengl.GL13.GL_MULTISAMPLE;

public class Lune extends GuiScreen {
    public static Module currentModul;
    public static ArrayList<Module> modArrayList = new ArrayList<>();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // Main Window
        RenderUtil.drawRoundedRect(windowX, windowY, windowX + windowWeight, windowY + windowHeight, color_main,
                color_main);
        // Middle
        RenderUtil.drawRoundedRect(windowX + 75, windowY, windowX + 200, windowY + windowHeight, color_middle,
                color_middle);
        // Left
        RenderUtil.drawRoundedRect(windowX, windowY, windowX + 75, windowY + windowHeight, color_left, color_left);

        FontLoaders.F30.drawString(Client.real_name, windowX + 15, windowY + 6, new Color(255, 255, 255).getRGB());

        int dWheel = Mouse.getDWheel();
        // Role the Modules
        if (isHovered(windowX + 80, windowY, windowX + 200, windowY + windowHeight, mouseX, mouseY)) {
            if (dWheel < 0 && role2 + windowY + 45
                    + ModuleManager.getModulesInType(currentcategory).size() * 30 > windowY + windowHeight) {
                role2 -= 8;
            }
        ...
        for (ModuleType mc : ModuleType.values()) {
            if (mc == currentcategory) {
                drawImage(windowX + 10, cateStartY + 5, 8, 8,
                        new ResourceLocation("LuneClientGui/" + mc.toString() + ".png"), new Color(255, 255, 255));
                // Render the line under the category
                RenderUtil.drawRect(windowX, windowY + currentY, windowX + 2f, windowY + currentY + 20, color_middle);
            } else {
                drawImage(windowX + 10, cateStartY + 5, 8, 8,
                        new ResourceLocation("LuneClientGui/" + mc.toString() + ".png"), new Color(220, 220, 220));
            }
                ...
                if (searchbox.getText().isEmpty()) {
                    if (m.getState()) {
                        if (isHovered(windowX + 80, realY, windowX + 195, realY + 20, mouseX, mouseY)) {
                            RenderUtil.drawRect(windowX + 80, realY, windowX + 195, realY + 20,
                        ...
                        if (isHovered(windowX + 80, realY, windowX + 195, realY + 20, mouseX, mouseY)) {
                            RenderUtil.drawRect((float) (windowX + 79.5), realY, windowX + 195.5F, realY + 20.5F,
                                    new Color(107, 127, 255).getRGB());
                        }
                        RenderUtil.drawRect(windowX + 80, realY, windowX + 195, realY + 20, color_main);
                        FontLoaders.F16.drawString(m.getName(), windowX + 90, realY + 8,
                    ...
                    if (m.getState()) {
                        RenderUtil.drawRoundRect(windowX + 80, realY, windowX + 195, realY + 20,
                                new Color(107, 127, 255).getRGB());
                        FontLoaders.F16.drawString(m.getName(), windowX + 90, realY + 8,
                                new Color(255, 255, 255).getRGB());
                    } else {
                        if (isHovered(windowX + 80, realY, windowX + 195, realY + 20, mouseX, mouseY)) {
                            RenderUtil.drawRoundRect((float) (windowX + 79.5), realY - 0.5F, windowX + 195.5F,
                                    realY + 20.5F, new Color(107, 127, 255).getRGB());
                        }
                        RenderUtil.drawRoundRect(windowX + 80, realY, windowX + 195, realY + 20, color_main);
                        FontLoaders.F16.drawString(m.getName(), windowX + 90, realY + 8,
                                new Color(180, 180, 180).getRGB());
        ...
        RenderUtil.drawRoundRect(windowX + 80, windowY, windowX + 200, windowY + windowHeight,
...
//            if (currentModul.getCategory() == M.Cloud) {
...
//            }
            ...
            for (Value v : currentModul.getValues()) {
                if (v instanceof Mode) {
                    FontLoaders.F14.drawString(v.getName(), windowX + 210, valueRealY, new Color(204, 204, 204).getRGB());
                    if (isHovered(windowX + windowWeight - 80, valueRealY - 5, windowX + windowWeight - 15,
                            valueRealY + 5, mouseX, mouseY)) {
                        RenderUtil.drawRect((float) (windowX + windowWeight - 80.5), (float) (valueRealY - 4.5),
                                (float) (windowX + windowWeight - 14.5), (float) (valueRealY + 7.5), color_select);
                        RenderUtil.drawRect(windowX + windowWeight - 80, valueRealY - 4, windowX + windowWeight - 15,
                    ...
                    if ((boolean) v.getValue()) {
                        if (isHovered(windowX + windowWeight - 36, valueRealY - 5, windowX + windowWeight - 15,
                                valueRealY + 5, mouseX, mouseY)) {
                            RenderUtil.drawRect(windowX + windowWeight - 36, valueRealY - 5,
                        ...
                        drawCircle(windowX + windowWeight - 20, valueRealY, 3,
                                new Color(255, 255, 255).getRGB());
                    } else {
                        if (isHovered(windowX + windowWeight - 36, valueRealY - 5, windowX + windowWeight - 15,
                                valueRealY + 5, mouseX, mouseY)) {
                            RenderUtil.drawRect((float) (windowX + windowWeight - 36.5), (float) (valueRealY - 5.5),
                                    (float) (windowX + windowWeight - 14.5), (float) (valueRealY + 5.5), color_select);
                        }
                        drawRect(windowX + windowWeight - 36, valueRealY - 5, windowX + windowWeight - 15,
                                valueRealY + 5, color_unselect);
                        drawCircle(windowX + windowWeight - 30, valueRealY, 3,
                    ...
                    if (isHovered(windowX + windowWeight - 15 - 67, valueRealY, windowX + windowWeight - 15,
                        ...
                        RenderUtil.drawRect(windowX + windowWeight - 15 - 67.5F, valueRealY - 0.5F,
                    ...
                    RenderUtil.drawRect(windowX + windowWeight - 15 - 67, valueRealY,
            ...
            RenderUtil.drawRoundRect(windowX + 200, windowY, windowX + windowWeight, windowY + windowHeight - 10,
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (currentModul != null) {
            int valueStartY = windowY + 25;
            int valueRealY = valueStartY + startValue;
            RenderUtil.drawRoundRect(windowX + 170, windowY + 45, windowX + windowWeight - 10,
