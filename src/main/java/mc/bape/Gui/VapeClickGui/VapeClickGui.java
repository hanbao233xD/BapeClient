...
import mc.bape.util.utils.RenderUtil;
import mc.bape.util.utils.RenderUtils;
...
//        }
    ...
    public float smoothTrans(double current, double last){
        return (float) (current + (last - current) / 10);
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (Client.FluxTheme) {
            RenderUtil.drawGradientRect(0, 0, sResolution.getScaledWidth(), sResolution.getScaledHeight(), new Color(255, 130, 164, 100).getRGB(), new Color(0, 0, 0, 30).getRGB());
        } else {
            RenderUtil.drawGradientRect(0, 0, sResolution.getScaledWidth(), sResolution.getScaledHeight(), Client.Theme, new Color(0, 0, 0, 30).getRGB());
        }
//        if (inAnim > 0) {
...
//        }
        ...
        for (Enum<?> e : ClickType.values()) {
            ...
            if (true) {
                if (e == selectType) {
                    if(e == ClickType.Home){
                        FontLoaders.F30.drawString(Client.name, typeXAnim[i] , windowY+15 , new Color(255, 255, 255).getRGB());
                        typeX += (32 + FontLoaders.F18.getStringWidth(e.name() + " "));
                   }
//                    } else if(e == ClickType.Config) {
////                        RenderUtils.drawImage(typeXAnim[i], windowY + 10, 16, 16, new ResourceLocation("vapeclickgui/config.png"), new Color(255, 255, 255));
//                        FontLoaders.F18.drawString(e.name(), typeXAnim[i] + 20, windowY + 15, new Color(255, 255, 255).getRGB());
//                        typeX += (32 + FontLoaders.F18.getStringWidth(e.name() + " "));
//                    }
                } else {
//                    RenderUtils.drawImage(typeXAnim[i], windowY + 10, 16, 16, new ResourceLocation("vapeclickgui/setting.png"), new Color(79, 80, 86));
                    typeX += (32);
                }
            } else {
//                RenderUtils.drawImage(windowX + width - 20, windowY + 10, 16, 16, new ResourceLocation("vapeclickgui/setting.png"), e == selectType ? new Color(255, 255, 255) : new Color(79, 80, 86));
            }
            i++;
        }
        ...
        if (selectType == ClickType.Home) {
            ...
            if (selectMod == null) {
                ...
                for (ModuleType m : ModuleType.values()) {
                    if (m == modCategory) {
                        ...
                        RenderUtils.drawRoundRect(windowX + 20, hy + FontLoaders.F18.getStringHeight("") + 2, windowX + 30, hy + FontLoaders.F18.getStringHeight("") + 4, Client.Theme);
                        if (isHovered(windowX, windowY, windowX + width, windowY + 20, mouseX, mouseY) && Mouse.isButtonDown(0)) {
                            hy = cateY;
                        } else {
                            if (hy != cateY) {
            ...
            if (selectMod != null) {
                ...
                RenderUtils.drawImage(windowX + 435 + valuemodx, windowY + 65, 16, 16, new ResourceLocation("vapeclickgui/back.png"), new Color(82, 82, 85));
                    ...
                    if (v instanceof Option) {
                        if (valuey + 4 > windowY + 100) {
                            if (((Boolean) v.getValue())) {
                                ...
                                v.optionAnim = 100;
                                RenderUtil.drawRoundedRect(windowX + width - 30, valuey + 2, windowX + width - 10, valuey + 12, 4, new Color(Client.ThemeR,Client.ThemeG,Client.ThemeB, (int) (v.optionAnimNow / 100 * 255)).getRGB());
                        ...
                        if (valuey + 4 > windowY + 100) {
                            ...
                            float present = (float) (((windowX + width - 11) - (windowX + 450 + valuemodx))
                                    * (((Number) v.getValue()).floatValue() - ((Numbers) v).getMinimum().floatValue())
                                    / (((Numbers) v).getMaximum().floatValue() - ((Numbers) v).getMaximum().floatValue()));
                            ...
                            RenderUtil.drawRect(windowX + 450 + valuemodx, valuey + 20, windowX + 450 + valuemodx + present, valuey + 21.5f, Client.Theme);
                            RenderUtils.drawCircle(windowX + 450 + valuemodx + present, valuey + 21f, 5, new Color(32, 31, 35).getRGB());
                            RenderUtils.drawCircle(windowX + 450 + valuemodx + present, valuey + 21f, 5, new Color(32, 31, 35).getRGB());
                            RenderUtils.drawCircle(windowX + 450 + valuemodx + present, valuey + 21f, 4, Client.Theme);
                            RenderUtils.drawCircle(windowX + 450 + valuemodx + present, valuey + 21f, 4, Client.Theme);
...
//                }
            ...
            for (Module m : ModuleManager.getModules()) {
                ...
                if (isHovered(windowX + 100 + valuemodx, modY - 10, windowX + 425 + valuemodx, modY + 25, mouseX, mouseY)) {
                    FontLoaders.F16.drawString(m.Descript + ".", windowX + 225 + valuemodx, modY + 5, new Color(94, 95, 98).getRGB());
                } else {
                    FontLoaders.F16.drawString(m.Descript + ".", windowX + 220 + valuemodx, modY + 5, new Color(94, 95, 98).getRGB());
                }

                if (m.getState()) {
                    FontLoaders.F18.drawString(m.getName(), windowX + 140 + valuemodx, modY + 5, new Color(220, 220, 220).getRGB());
                    RenderUtils.drawRoundRect(windowX + 100 + valuemodx, modY - 10, windowX + 125 + valuemodx, modY + 25, new Color(Client.ThemeR, Client.ThemeG,Client.ThemeB, (int) (m.optionAnimNow / 100f * 255)).getRGB());
                    RenderUtils.drawImage(windowX + 105 + valuemodx, modY, 16, 16, new ResourceLocation("vapeclickgui/module.png"), new Color(220, 220, 220));
                    m.optionAnim = 100;

//                    RenderUtil.drawRoundedRect(windowX + 380 + valuemodx, modY + 2, windowX + 400 + valuemodx, modY + 12, 4, new Color(Client.ThemeR, Client.ThemeG,Client.ThemeB, (int) (m.optionAnimNow / 100f * 255)).getRGB());
//                    RenderUtils.drawCircle(windowX + 385 + 10 * m.optionAnimNow / 100 + valuemodx, modY + 7, 3.5, new Color(255, 255, 255).getRGB());
                    ...
                    RenderUtils.drawImage(windowX + 105 + valuemodx, modY, 16, 16, new ResourceLocation("vapeclickgui/module.png"), new Color(92, 90, 94));
                    m.optionAnim = 0;
...
//                    RenderUtils.drawCircle(windowX + 385 + 10 * m.optionAnimNow / 100 + valuemodx, modY + 7, 3.5, new Color(59, 60, 65).getRGB());
        ...
        if (false) {
            ...
            RenderUtils.drawRoundRect(windowX + 20, windowY + FontLoaders.F18.getStringHeight("") + 62, windowX + 30, windowY + FontLoaders.F18.getStringHeight("") + 64, Client.Theme);
            ...
            RenderUtils.drawCircle(windowX + 100, windowY + 100, 5, Client.Theme);
            RenderUtils.drawCircle(windowX + 100, windowY + 100, 5, Client.Theme);
...
//                RenderUtils.drawImage(cx + 10, cy + 80, 10, 8, new ResourceLocation("Cloud.png"), new Color(91, 92, 98));
                FontLoaders.F14.drawString(c.description, cx + 25, cy + 82, new Color(86, 85, 88).getRGB());
                if (c.equals(Client.config)) {
                    FontLoaders.F14.drawString("Using", cx + 70, cy + 82, Client.Theme);
                }


                if (isHovered(cx, cy, cx + 100, cy + 100, mouseX, mouseY) && Mouse.isButtonDown(0) && valuetimer.delay(500)) {
                    Client.config = String.valueOf(c);
            ...
            if (isHovered(windowX + 95, windowY + 95, windowX + 105, windowY + 105, mouseX, mouseY) && Mouse.isButtonDown(0) && valuetimer.delay(500) && configs.size() < 6) {
                ...
                for (Config c : configs) {
                    if (c.name.equals(configInputBox.getText())) {
                    	Client.config = configInputBox.getText();
                        try {
                            Client.SaveConfig();
                        } catch (IOException e) {
                            e.printStackTrace();
                ...
                Helper.sendMessage("Config failed.");
                valuetimer.reset();
            }
